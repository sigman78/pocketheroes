...
/** @file
 *	@brief The FastArray template implements a generic dynamic array container
 */
/** @addtogroup iolib_he
 *  @{
 */
#ifndef _IOLIB_HELIUM_CONTAINER_FAST_ARRAY_H_
#define _IOLIB_HELIUM_CONTAINER_FAST_ARRAY_H_
...
// DynamicArray
// Constructors
template <class T, uint32 SIZ> 
iFastArray<T,SIZ>::iFastArray()
{
	Reset();
}	
...
// Take a copy
template <class T, uint32 SIZ> 
iFastArray<T,SIZ>::iFastArray( const iFastArray& rvalue )
{
	Reset();
	operator += ( rvalue );
}
...
// Assignment
template <class T, uint32 SIZ> 
iFastArray<T,SIZ>& iFastArray<T,SIZ>::operator = ( const iFastArray& rvalue )
{
	if ( this ==&rvalue ) {
		return *this;
	}
...
iFastArray<T,SIZ>& iFastArray<T,SIZ>::operator += ( const iFastArray& rvalue )
{
	...
	else 
	{
		...
		// Copy elements
		CopyBlock8( &head->count, &curr->count, curr->count*sizeof(T)+sizeof(uint32));
	...
	do 
	{
		...
		// Copy elements
		CopyBlock8( &newChunk->count, &curr->count, curr->count*sizeof(T)+sizeof(uint32));		
...
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::Add( T item )
{
...
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::AddUnique( T item )
{
...
template <class T, uint32 SIZ> 
T iFastArray<T,SIZ>::Remove( T sentinal )
{
	Chunk* c;
...
//
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::Push( T item )
{
	...
	else 
	{
		...
		else 
		{
			...
			MoveBlock8( (uint8*)&head->data[1], (uint8*)&head->data[0], head->count*sizeof(T) );
		}
	}
...
//
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::PushUnique( T item )
{
...
//
template <class T, uint32 SIZ> 
T iFastArray<T,SIZ>::Pop( T sentinal )
{
...
// 
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Insert( T node )
{
	if ( !head ) 
	{
		Add( node );
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::InsertUnique( T node )
{
	if ( !head ) 
	{
		Add( node );
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Insert( const Iterator& beforeHere, T item )
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::InsertAfter( const Iterator& afterHere, T item )
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::InsertBefore( const Iterator& beforeHere, T item )
{
	...
	if ( c->count == SIZ ) 
	{
		...
		MoveBlock8((uint8*)&nextChunk->data[0], (uint8*)&c->data[SIZ/2], (SIZ-SIZ/2)*sizeof(T) );
	...
	MoveBlock8( (uint8*)&c->data[index+1], (uint8*)&c->data[index], (c->count-index)*sizeof(T) );
	// add the new node
	c->data[index] = item;
	c->count++;
...
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::Remove( const Iterator& fit,  const Iterator& lit)
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Remove( const Iterator& it )
{
	...
	if ( node < last ) {
		// fill da gap
		MoveBlock8( (uint8*)node, (uint8*)node + sizeof(T), (last - node) * sizeof(T) );
	}
...
// 
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Remove( uint32 n )
{
	Iterator it = First();
	it += n;
	return Remove( it );
...
// remove last 'n' items
template <class T, uint32 SIZ>
void iFastArray<T,SIZ>::RemoveLast( uint32 n )
{
	if ( head ) 
	{
...
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::RemovePack( const Iterator& it )
{
	Chunk* c = it.chunk;
...
//
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::RemoveAll()
{
	if ( head ) 
	{
...
//
template <class T, uint32 SIZ> 
iFastArray<T,SIZ>::operator bool() const
{
...
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Index( uint32 index )
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::ConstIterator iFastArray<T,SIZ>::Index( uint32 index ) const
{
	if ( index >= Count() )	return End();
...
template <class T, uint32 SIZ> 
T& iFastArray<T,SIZ>::operator[] ( uint32 index )
{
...
//
template <class T, uint32 SIZ> 
const T& iFastArray<T,SIZ>::operator[] ( uint32 index ) const
{
	check( index < GetSize() );//, ("Out of bounds, index=%u", index));
...
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::First()
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::ConstIterator iFastArray<T,SIZ>::First() const
{
...
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::Last()
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::ConstIterator iFastArray<T,SIZ>::Last() const
{
...
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::Iterator iFastArray<T,SIZ>::End()
{
...
//
template <class T, uint32 SIZ> 
typename iFastArray<T,SIZ>::ConstIterator iFastArray<T,SIZ>::End() const
{
...
template <class T, uint32 SIZ> 
uint32 iFastArray<T,SIZ>::GetSize() const
{
	uint32 count = 0;
	Chunk* c = head;
...
//
template <class T, uint32 SIZ> 
bool iFastArray<T,SIZ>::IsEmpty() const
{
	return head == NULL;
}
...
template <class T, uint32 SIZ> 
uint32 iFastArray<T,SIZ>::Index( Iterator it ) const
{
...
//
template <class T, uint32 SIZ> 
uint32 iFastArray<T,SIZ>::Index( ConstIterator it ) const
{
	uint32 count = 0;
...
template <class T, uint32 SIZ> 
bool iFastArray<T,SIZ>::Exists( const Iterator& node ) const
{
...
// Swap two elements
template <class T, uint32 SIZ> 
void iFastArray<T,SIZ>::Swap( Iterator& it1, Iterator& it2 )
{
...
#endif //_IOLIB_HELIUM_CONTAINER_FAST_ARRAY_H_
/** @} */ // iolib_he
